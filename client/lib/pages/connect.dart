...
import 'package:client/pages/prejoin.dart';
...
class _ConnectPageState extends State<ConnectPage> {
  ...
  Future<String> fetchToken() async {
    ...
    try {
      var response = await http.get(Uri.parse(
          'https://komaki.tech/api/get_token?roomName=$roomName&identity=$identity'));
  ...
  Future<void> _connect(BuildContext ctx) async {
    ...
    try {
      ...
      var url = 'wss://tentwenty-bp8gb2jg.livekit.cloud';
      // instead of inputing the token by hand, we need to fetch the token from the server
...
String generateRandomIdentity() {
  ...
  final random = Random();
  final randomValue = random.nextInt(999999); // Generates a random integer
